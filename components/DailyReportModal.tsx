...
import { X, CheckCircle, Clock, Copy, Check, Table, ClipboardCheck, User, Users, Tag } from 'lucide-react';
...
const DailyReportModal: React.FC<DailyReportModalProps> = ({ manuscripts, onClose, onMarkReported }) => {
  ...
  const selectedFiles = todayFiles;
  ...
  const handleFinish = () => {
    onMarkReported(selectedFiles.map(m => m.id));
    handleClose();
  };
      ...
      <div className={`bg-white rounded-2xl shadow-2xl w-full max-w-4xl flex flex-col max-h-[90vh] overflow-hidden border border-slate-200 ${isClosing ? 'modal-content-exit' : 'modal-content-enter'}`}>
        ...
        <div className="flex justify-between items-center p-6 border-b border-slate-100 bg-indigo-50/30">
          <div className="flex items-center gap-3">
            <div className="p-2 rounded-xl shadow-lg transition-colors bg-emerald-600 shadow-emerald-200">
              <Table className="w-6 h-6 text-white" />
            </div>
            <div>
              <h2 className="text-xl font-bold text-slate-800">End of Day Draft</h2>
              <p className="text-sm text-slate-500">Ready to be pasted into your email client</p>
            </div>
          </div>
