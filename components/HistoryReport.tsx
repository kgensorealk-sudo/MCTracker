...
import { TrendingUp, Clock, FileText, History, Search, ClipboardList, FileSearch, AlertCircle, BarChart3, Coins, DollarSign, Wallet, Settings2, AlertTriangle, CheckCircle, FileCheck, CalendarDays, Calendar, X } from 'lucide-react';
...
interface HistoryReportProps {
  ...
  onUpdateRate: (id: string, rates: { usd: number; php: number }) => void;
}
...
const HistoryReport: React.FC<HistoryReportProps> = ({ manuscripts, onBulkUpdate, cycleRates, onUpdateRate }) => {
  ...
  const currentCycle = useCycleDates();

  const allJournals = useMemo(() => 
    Array.from(new Set(manuscripts.map(m => m.journalCode).filter(Boolean))).sort()
  , [manuscripts]);
  ...
  const stats = useMemo(() => {
    ...
    const journalStats: Record<string, { count: number; totalTat: number; countWithTat: number }> = {};
    ...
    manuscripts.forEach(m => {
      if (m.status === Status.WORKED || m.status === Status.BILLED) {
        const workDate = m.billedDate || m.completedDate || m.dateStatusChanged || m.dateUpdated || m.dateReceived;
        ...
        if (m.journalCode) {
            if (!journalStats[m.journalCode]) journalStats[m.journalCode] = { count: 0, totalTat: 0, countWithTat: 0 };
            journalStats[m.journalCode].count += 1;
            const start = new Date(m.dateReceived).getTime();
            const end = new Date(workDate).getTime();
            if (!isNaN(start) && !isNaN(end)) {
                const days = Math.max(0, (end - start) / (1000 * 3600 * 24));
                journalStats[m.journalCode].totalTat += days;
                journalStats[m.journalCode].countWithTat += 1;
            }
        }
      }
    });
    ...
    const journalChartData = Object.entries(journalStats)
      .map(([code, data]) => ({
        code,
        count: data.count,
        avgTat: data.countWithTat > 0 ? parseFloat((data.totalTat / data.countWithTat).toFixed(1)) : 0
      }))
      .filter(j => selectedJournals.length === 0 || selectedJournals.includes(j.code))
  ...
  const updateSelectedCycleRate = (key: keyof RateProfile, value: number) => {
    ...
    onUpdateRate(selectedCycleId, { ...current, [key]: value });
  };
            ...
            <div className="flex flex-wrap items-center gap-4">
               <div className="relative flex-1 min-w-[200px] max-w-md">
                  ...
                  <input 
                     ...
                     className="w-full pl-10 pr-10 py-2 text-sm border-slate-200 rounded-xl bg-white focus:ring-2 focus:ring-indigo-500 font-semibold text-slate-700"
                     value={searchTerm}
               ...
               {selectedManuscriptIds.length > 0 && (
                  <button 
                     onClick={() => {
                        onBulkUpdate(selectedManuscriptIds, { status: Status.BILLED });
                        setSelectedManuscriptIds([]);
                     }}
                     className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-xl text-xs font-bold shadow-lg shadow-emerald-100 transition-all hover:scale-105 active:scale-95"
                  >
                     <FileCheck className="w-4 h-4" />
                     Mark {selectedManuscriptIds.length} as Billed
                  </button>
               )}
                        ...
                        <input 
                           ...
                           checked={
                              selectedCycleId && stats.cycleGroups[selectedCycleId] 
                                 ? stats.cycleGroups[selectedCycleId].files.filter(f => f.status !== Status.BILLED).length > 0 &&
                                   stats.cycleGroups[selectedCycleId].files.filter(f => f.status !== Status.BILLED).every(f => selectedManuscriptIds.includes(f.id))
                                 : false
                           }
                           onChange={(e) => {
                              if (selectedCycleId && stats.cycleGroups[selectedCycleId]) {
                                 const unbilledIds = stats.cycleGroups[selectedCycleId].files
                                    .filter(f => f.status !== Status.BILLED)
                                    .map(f => f.id);
                                 if (e.target.checked) {
                                    setSelectedManuscriptIds(prev => Array.from(new Set([...prev, ...unbilledIds])));
                                 } else {
                                    setSelectedManuscriptIds(prev => prev.filter(id => !unbilledIds.includes(id)));
                                 }
                              }
                           }}
                        />
               ...
               <tbody className="divide-y divide-slate-100">
                  {!selectedCycleId || !stats.cycleGroups[selectedCycleId] ? (
                     ...
                     stats.cycleGroups[selectedCycleId].files.map((m) => {
                        ...
                        const isSelected = selectedManuscriptIds.includes(m.id);
                        const rates = cycleRates[selectedCycleId] || DEFAULT_RATES;
                        return (
                           <tr key={m.id} className={`transition-colors group ${isBilled ? 'hover:bg-slate-50/80' : isSelected ? 'bg-indigo-50/50 hover:bg-indigo-50/70' : 'bg-rose-50/20 hover:bg-rose-50/40'}`}>
                              <td className="px-4 py-4">
                                 <input 
                                    ...
                                    onChange={() => {
                                       if (isSelected) {
                                          setSelectedManuscriptIds(prev => prev.filter(id => id !== m.id));
                                       } else {
                                          setSelectedManuscriptIds(prev => [...prev, m.id]);
                                       }
                                    }}
                                 />
                              </td>
                              ...
                              <td className="px-6 py-4">
                                 <span className="text-slate-600 font-bold bg-slate-50 px-2 py-0.5 rounded">
                                    {new Date(m.completedDate || m.dateStatusChanged || m.dateUpdated || m.dateReceived || Date.now()).toLocaleDateString()}
                                 </span>
                              </td>
                              <td className="px-6 py-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 flex flex-col">
           <div className="flex items-center justify-between mb-6">
              <div>
                 <h3 className="font-bold text-slate-800 flex items-center gap-2">
                    <BarChart3 className="w-5 h-5 text-indigo-500" /> Journal Performance
                 </h3>
                 <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mt-1">Top journals by volume & speed</p>
              </div>
              <div className="relative">
                 <button 
                    onClick={() => setIsJournalFilterOpen(!isJournalFilterOpen)}
                    className={`p-2 rounded-lg border transition-all flex items-center gap-2 text-[10px] font-black uppercase tracking-widest ${selectedJournals.length > 0 ? 'bg-indigo-50 border-indigo-200 text-indigo-600' : 'bg-white border-slate-200 text-slate-400 hover:bg-slate-50'}`}
                 >
                    <Search className="w-3 h-3" />
                    {selectedJournals.length > 0 ? `${selectedJournals.length} Selected` : 'Filter'}
                 </button>
                 ...
                 <AnimatePresence>
                    {isJournalFilterOpen && (
                       <motion.div 
                          ...
                          className="absolute right-0 top-full mt-2 w-64 bg-white border border-slate-200 rounded-2xl shadow-2xl z-50 p-4"
                       >
                          <div className="flex items-center justify-between mb-3 pb-2 border-b border-slate-100">
                             <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Select Journals</span>
                          ...
                          <div className="max-h-60 overflow-y-auto custom-scrollbar space-y-1">
                             {allJournals.map(code => (
                                <label key={code} className="flex items-center gap-3 p-2 hover:bg-slate-50 rounded-xl cursor-pointer transition-colors">
                                   <input 
                                      type="checkbox"
                                      className="w-4 h-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500"
                                      checked={selectedJournals.includes(code)}
                                      onChange={() => {
                                         if (selectedJournals.includes(code)) {
                                            setSelectedJournals(selectedJournals.filter(c => c !== code));
                                         } else {
                                            setSelectedJournals([...selectedJournals, code]);
                                         }
                                      }}
                                   />
                                   <span className="text-xs font-bold text-slate-700 uppercase">{code}</span>
                                </label>
                             ))}
                          </div>
                       </motion.div>
           ...
           <div className="flex-1 space-y-4 overflow-y-auto max-h-[300px] pr-2 custom-scrollbar">
              {stats.journalChartData.length === 0 ? (
                 <div className="h-full flex flex-col items-center justify-center text-slate-400 py-12">
                    <p className="text-xs font-bold">No data available</p>
                 </div>
              ) : (
                 stats.journalChartData.map((j, idx) => (
                    <div key={j.code} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100 group hover:border-indigo-200 transition-all">
                       <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-lg bg-white border border-slate-200 flex items-center justify-center text-[10px] font-black text-slate-500 group-hover:text-indigo-600 transition-colors">
                             {idx + 1}
                          </div>
                          <div>
                             <p className="text-xs font-black text-slate-800 uppercase tracking-tight">{j.code}</p>
                             <p className="text-[10px] text-slate-400 font-bold">{j.count} files handled</p>
                          </div>
                       </div>
                       <div className="text-right">
                          <p className={`text-[10px] font-black px-2 py-0.5 rounded-lg border ${
                             j.avgTat <= 2 ? 'bg-emerald-50 text-emerald-700 border-emerald-100' :
                             j.avgTat <= 5 ? 'bg-amber-50 text-amber-700 border-amber-100' :
