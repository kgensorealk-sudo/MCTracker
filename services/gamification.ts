...
export const ALL_DAILY_QUESTS: Quest[] = [
  {
    id: 'daily_grind',
    title: 'Daily Grind',
  ...
  },
  {
    ...
    description: 'Work on 3 different journals today.',
    target: 3,
    rewardXP: 150,
    progress: (mss) => {
...
export const getDailyQuest = (date: Date = new Date()): Quest => {
    // Use date as seed
    const seed = date.getFullYear() * 10000 + (date.getMonth() + 1) * 100 + date.getDate();
    const index = seed % ALL_DAILY_QUESTS.length;
    return ALL_DAILY_QUESTS[index];
};

// For backward compatibility and XP calculation
export const DAILY_QUESTS = ALL_DAILY_QUESTS;
...
export const ACHIEVEMENTS: Achievement[] = [
  // --- EXISTING & BASE MILESTONES ---
  {
    ...
    maxProgressValue: 1,
    currentProgressValue: (mss) => mss.filter(m => m.status === Status.WORKED || m.status === Status.BILLED).length,
    condition: (mss) => mss.some(m => m.status === Status.WORKED || m.status === Status.BILLED),
    progress: (mss) => Math.min(100, (mss.filter(m => m.status === Status.WORKED || m.status === Status.BILLED).length / 1) * 100)
  },
  {
    id: 'novice',
    ...
    maxProgressValue: 5,
    currentProgressValue: (mss) => mss.filter(m => m.status === Status.WORKED || m.status === Status.BILLED).length,
    condition: (mss) => mss.filter(m => m.status === Status.WORKED || m.status === Status.BILLED).length >= 5,
    progress: (mss) => Math.min(100, (mss.filter(m => m.status === Status.WORKED || m.status === Status.BILLED).length / 5) * 100)
  },
  {
    id: 'getting_started',
    ...
    maxProgressValue: 25,
    currentProgressValue: (mss) => mss.filter(m => m.status === Status.WORKED || m.status === Status.BILLED).length,
    condition: (mss) => mss.filter(m => m.status === Status.WORKED || m.status === Status.BILLED).length >= 25,
    progress: (mss) => Math.min(100, (mss.filter(m => m.status === Status.WORKED || m.status === Status.BILLED).length / 25) * 100)
  },
  {
    id: 'warming_up',
    title: 'Warming Up',
    ...
    maxProgressValue: 50,
    currentProgressValue: (mss) => mss.filter(m => m.status === Status.WORKED || m.status === Status.BILLED).length,
    condition: (mss) => mss.filter(m => m.status === Status.WORKED || m.status === Status.BILLED).length >= 50,
    progress: (mss) => Math.min(100, (mss.filter(m => m.status === Status.WORKED || m.status === Status.BILLED).length / 50) * 100)
  },
  {
    id: 'century_club',
    title: 'Century Club',
    ...
    currentProgressValue: (mss) => mss.filter(m => m.status === Status.WORKED || m.status === Status.BILLED).length,
    condition: (mss) => mss.filter(m => m.status === Status.WORKED || m.status === Status.BILLED).length >= 100,
    progress: (mss) => Math.min(100, (mss.filter(m => m.status === Status.WORKED || m.status === Status.BILLED).length / 100) * 100)
  },
  {
    id: 'analyst_pro',
    title: 'Analyst Pro',
    ...
    currentProgressValue: (mss) => mss.filter(m => m.status === Status.WORKED || m.status === Status.BILLED).length,
    condition: (mss) => mss.filter(m => m.status === Status.WORKED || m.status === Status.BILLED).length >= 250,
    progress: (mss) => Math.min(100, (mss.filter(m => m.status === Status.WORKED || m.status === Status.BILLED).length / 250) * 100)
  },
  {
    id: 'half_k',
    title: 'Half-K',
    ...
    currentProgressValue: (mss) => mss.filter(m => m.status === Status.WORKED || m.status === Status.BILLED).length,
    condition: (mss) => mss.filter(m => m.status === Status.WORKED || m.status === Status.BILLED).length >= 500,
    progress: (mss) => Math.min(100, (mss.filter(m => m.status === Status.WORKED || m.status === Status.BILLED).length / 500) * 100)
  },
  {
    id: 'master_analyst',
    ...
    currentProgressValue: (mss) => mss.filter(m => m.status === Status.WORKED || m.status === Status.BILLED).length,
    condition: (mss) => mss.filter(m => m.status === Status.WORKED || m.status === Status.BILLED).length >= 1000,
    progress: (mss) => Math.min(100, (mss.filter(m => m.status === Status.WORKED || m.status === Status.BILLED).length / 1000) * 100)
  },
  {
    id: 'kilo_plus',
    title: 'Veteran',
    ...
    currentProgressValue: (mss) => mss.filter(m => m.status === Status.WORKED || m.status === Status.BILLED).length,
    condition: (mss) => mss.filter(m => m.status === Status.WORKED || m.status === Status.BILLED).length >= 1500,
    progress: (mss) => Math.min(100, (mss.filter(m => m.status === Status.WORKED || m.status === Status.BILLED).length / 1500) * 100)
  },
  {
    id: 'two_grand',
    title: 'Two Grand',
    ...
    currentProgressValue: (mss) => mss.filter(m => m.status === Status.WORKED || m.status === Status.BILLED).length,
    condition: (mss) => mss.filter(m => m.status === Status.WORKED || m.status === Status.BILLED).length >= 2000,
    progress: (mss) => Math.min(100, (mss.filter(m => m.status === Status.WORKED || m.status === Status.BILLED).length / 2000) * 100)
  },
  {
    id: 'five_k',
    title: 'High Five',
    ...
    currentProgressValue: (mss) => mss.filter(m => m.status === Status.WORKED || m.status === Status.BILLED).length,
    condition: (mss) => mss.filter(m => m.status === Status.WORKED || m.status === Status.BILLED).length >= 5000,
    progress: (mss) => Math.min(100, (mss.filter(m => m.status === Status.WORKED || m.status === Status.BILLED).length / 5000) * 100)
  },
  ...
  // --- QUALITY (Straight Through) ---
  {
    ...
    xpReward: XP_PER_ACHIEVEMENT_TIER.BRONZE,
    maxProgressValue: 10,
    currentProgressValue: (mss) => mss.filter(m => (m.status === Status.WORKED || m.status === Status.BILLED) && !m.queryReason).length,
    condition: (mss) => mss.filter(m => (m.status === Status.WORKED || m.status === Status.BILLED) && !m.queryReason).length >= 10,
    progress: (mss) => Math.min(100, (mss.filter(m => (m.status === Status.WORKED || m.status === Status.BILLED) && !m.queryReason).length / 10) * 100)
  },
  {
    ...
    maxProgressValue: 50,
    currentProgressValue: (mss) => mss.filter(m => (m.status === Status.WORKED || m.status === Status.BILLED) && !m.queryReason).length,
    condition: (mss) => mss.filter(m => (m.status === Status.WORKED || m.status === Status.BILLED) && !m.queryReason).length >= 50,
    progress: (mss) => Math.min(100, (mss.filter(m => (m.status === Status.WORKED || m.status === Status.BILLED) && !m.queryReason).length / 50) * 100)
  },
  {
    id: 'quality_pristine',
    ...
    maxProgressValue: 100,
    currentProgressValue: (mss) => mss.filter(m => (m.status === Status.WORKED || m.status === Status.BILLED) && !m.queryReason).length,
    condition: (mss) => mss.filter(m => (m.status === Status.WORKED || m.status === Status.BILLED) && !m.queryReason).length >= 100,
    progress: (mss) => Math.min(100, (mss.filter(m => (m.status === Status.WORKED || m.status === Status.BILLED) && !m.queryReason).length / 100) * 100)
  },
  ...
  // --- QUERIES & PENDING ---
  {
    ...
    xpReward: XP_PER_ACHIEVEMENT_TIER.BRONZE,
    maxProgressValue: 10,
    currentProgressValue: (mss) => mss.filter(m => [Status.PENDING, Status.PENDING_JM, Status.PENDING_TL, Status.PENDING_CED].includes(m.status)).length,
    condition: (mss) => mss.filter(m => [Status.PENDING, Status.PENDING_JM, Status.PENDING_TL, Status.PENDING_CED].includes(m.status)).length >= 10,
    progress: (mss) => Math.min(100, (mss.filter(m => [Status.PENDING, Status.PENDING_JM, Status.PENDING_TL, Status.PENDING_CED].includes(m.status)).length / 10) * 100)
  },
  {
    id: 'query_master',
    title: 'Query Master',
    ...
    maxProgressValue: 100,
    currentProgressValue: (mss) => mss.filter(m => [Status.PENDING, Status.PENDING_JM, Status.PENDING_TL, Status.PENDING_CED].includes(m.status)).length,
    condition: (mss) => mss.filter(m => [Status.PENDING, Status.PENDING_JM, Status.PENDING_TL, Status.PENDING_CED].includes(m.status)).length >= 100,
  ...
  },
  {
    ...
    maxProgressValue: 500,
    currentProgressValue: (mss) => mss.filter(m => [Status.PENDING, Status.PENDING_JM, Status.PENDING_TL, Status.PENDING_CED].includes(m.status)).length,
    condition: (mss) => mss.filter(m => [Status.PENDING, Status.PENDING_JM, Status.PENDING_TL, Status.PENDING_CED].includes(m.status)).length >= 500,
    progress: (mss) => Math.min(100, (mss.filter(m => [Status.PENDING, Status.PENDING_JM, Status.PENDING_TL, Status.PENDING_CED].includes(m.status)).length / 500) * 100)
  },
  {
    id: 'query_messenger',
    title: 'The Messenger',
  ...
  // --- PRIORITIES ---
  {
    ...
    xpReward: XP_PER_ACHIEVEMENT_TIER.BRONZE,
    maxProgressValue: 10,
    currentProgressValue: (mss) => mss.filter(m => m.priority === 'Urgent' && (m.status === Status.WORKED || m.status === Status.BILLED)).length,
    condition: (mss) => mss.filter(m => m.priority === 'Urgent' && (m.status === Status.WORKED || m.status === Status.BILLED)).length >= 10,
    progress: (mss) => Math.min(100, (mss.filter(m => m.priority === 'Urgent' && (m.status === Status.WORKED || m.status === Status.BILLED)).length / 10) * 100)
  },
  {
    ...
    maxProgressValue: 50,
    currentProgressValue: (mss) => mss.filter(m => m.priority === 'Urgent' && (m.status === Status.WORKED || m.status === Status.BILLED)).length,
    condition: (mss) => mss.filter(m => m.priority === 'Urgent' && (m.status === Status.WORKED || m.status === Status.BILLED)).length >= 50,
    progress: (mss) => Math.min(100, (mss.filter(m => m.priority === 'Urgent' && (m.status === Status.WORKED || m.status === Status.BILLED)).length / 50) * 100)
  },
  {
    id: 'priority_hero',
    title: 'Hero',
    ...
    maxProgressValue: 100,
    currentProgressValue: (mss) => mss.filter(m => m.priority === 'Urgent' && (m.status === Status.WORKED || m.status === Status.BILLED)).length,
    condition: (mss) => mss.filter(m => m.priority === 'Urgent' && (m.status === Status.WORKED || m.status === Status.BILLED)).length >= 100,
    progress: (mss) => Math.min(100, (mss.filter(m => m.priority === 'Urgent' && (m.status === Status.WORKED || m.status === Status.BILLED)).length / 100) * 100)
  },
  ...
  // --- TIME & HABITS ---
  {
    ...
    xpReward: XP_PER_ACHIEVEMENT_TIER.BRONZE,
    maxProgressValue: 1,
    currentProgressValue: (mss) => mss.some(m => (m.status === Status.WORKED || m.status === Status.BILLED) && new Date(m.completedDate || '').getHours() < 8) ? 1 : 0,
    condition: (mss) => mss.some(m => (m.status === Status.WORKED || m.status === Status.BILLED) && new Date(m.completedDate || '').getHours() < 8),
    progress: (mss) => mss.some(m => (m.status === Status.WORKED || m.status === Status.BILLED) && new Date(m.completedDate || '').getHours() < 8) ? 100 : 0
  },
  {
    id: 'night_owl',
    title: 'Night Owl',
  ...
  },
  {
    ...
    currentProgressValue: (mss) => mss.some(m => {
       ...
       return (m.status === Status.WORKED || m.status === Status.BILLED) && (d.getDay() === 0 || d.getDay() === 6);
    }) ? 1 : 0,
    condition: (mss) => mss.some(m => {
       const d = new Date(m.completedDate || '');
       return (m.status === Status.WORKED || m.status === Status.BILLED) && (d.getDay() === 0 || d.getDay() === 6);
...
export const calculateLevel = (currentXP: number): UserLevel => {
    ...
    return {
        level,
        title,
        nextRankTitle: TITLES[currentTitleIndex + 1]?.title,
        currentXP,
        nextLevelXP,
        progressPercent
    };
